...
| [`kiln-next`](plugins/kiln-next/) | Classes for the Kiln runtime, under the holding name kiln-next until the old kiln plugin retires. Ships five Classes. The Crafter changes the artifact. The Designer is the `shape` skill: a three-lens design dialogue that ends at a written decision. The Planner turns a decision into a ticket plan with rerunnable checks. The Inspector rules once per build job on conformance to the plan and names the consequence it observed. The Prospector is a bounded researcher that invokes the research skill by name. | Agent Classes dispatched by Class (`kiln-next:crafter`, `kiln-next:planner`, `kiln-next:inspector`, `kiln-next:prospector`) from a job plan by the fire skill. When the fire skill hands it a design step, the Designer runs in the main thread as `/kiln-next:shape`. |
