...
| [`kiln-next`](plugins/kiln-next/) | Classes for the Kiln runtime, under the holding name kiln-next until the old kiln plugin retires. Ships the Crafter (changes the artifact), the Designer (the `shape` skill: a main-thread design dialogue that ends at a written decision), the Planner (turns a decision into a ticket plan with rerunnable checks), and the Inspector (rules once per build job on conformance to the plan). | Agent Classes dispatched by Class (`kiln-next:crafter`, `kiln-next:planner`, `kiln-next:inspector`) from a job plan by the fire skill; the Designer runs in the main thread as `/kiln-next:shape` when the fire skill hands it a design step |
