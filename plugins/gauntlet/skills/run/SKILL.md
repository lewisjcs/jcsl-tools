...
- `code-pr`: code-quality-audit (audit/inline), adversarial-review lane via gauntlet2-adversarial-review (family=code-diff).
...
- `code-local`: code-quality-audit, adversarial-review lane via gauntlet2-adversarial-review (family=code-diff). Same dispatch pair as `code-pr`; both lanes operate on diff content rather than GitHub metadata. No PR body dispatch (no PR exists for `code-local`).
- `plan`: plan-review, adversarial-review lane via gauntlet2-adversarial-review (family=plan-text — wires up plan-review's Architectural-risk lens per Phase 7 §9 resolution)
- `doc`: doc-review, adversarial-review lane via gauntlet2-adversarial-review (family=doc-text — wires up doc-review's Hidden-assumptions lens per Phase 7 §9 resolution)
- `directive`: directive-review (typed; `Skill:` black-box dispatch) + adversarial-review lane via gauntlet2-adversarial-review (the runtime has no `directive-text` family; pass family=doc-text as the closest fit and note the substitution in the trust-signal footer). No PR body dispatch. **For multi-file directive runs:** before dispatching any directive-review sub-tasks, count the files to be reviewed and emit a single line to chat: `"Dispatching directive-review for N file(s): [list]. This will spawn N×2 agent passes."` Then proceed without gating.
...
| `adversarial-review` lane | Yes (runtime-driven `gauntlet2-adversarial-finder` + `gauntlet2-adversarial-validator`) | **`Skill:` dispatch of `gauntlet:gauntlet2-adversarial-review` (v2, default).** The skill drives the deterministic runtime handshake (bundle → init → dispatch → receipt → result) inline; consume `<runDir>/result.json` `findings` as the lane's survivors — they arrive pre-adjudicated (severity-stratified gate) with `disposition: "survives"`. **Fallback:** if the v2 preflight fails (`node --version` absent or major < 22), dispatch `Skill: gauntlet:adversarial-review` (v1) instead and record `adversarial-review: v1 fallback (v2 preflight failed)` in the trust-signal footer. |
...
2. Each entry has one of three shapes:
   ...
   - **Adversarial-lane output (v2 default):** read `<runDir>/result.json`. `findings` is the survivors array; each item already carries `lens`, `location`, `claim`, `evidence`, `severity`, `category`, `confidence`, `recommendation`, `disposition: "survives"`. Promotion to the canonical 10-field shape (Phase 3 substep 1) only adds `skill: adversarial-review` and `verdict: "survives"` and applies the lens relabel — never re-gate these findings (the runtime's severity-stratified adjudication already did). If `executionStatus` is `failed`/`incomplete` or the run recorded a stage gap, mark the lane `review failed` in the trust-signal footer and continue (existing failed-domain rule). If the v1 fallback ran instead, its output is the 5-field shape (`lens`, `location`, `claim`, `evidence`, `severity`) plus `verdict` + `confidence` from the Validator, promoted to the 10-field shape during Phase 3 substep 1 as before.
...
1. **Concatenate** — Combine all findings from Phase 1 (per-domain) and Phase 2 (security) into a single array. Promote adversarial-lane findings to the 10-field canonical shape (v2 result items: add `skill`/`verdict` and relabel the lens; v1-fallback items: full 5-field promotion), and apply the §4.3 skill-audit transformation to any skill-audit findings. **Load [reference.md](reference.md) now** — it holds the field-by-field promotion rules (`skill`, `lens` via the 9-row mapping table, `category`, `recommendation`) and the lens-mapping needed for this substep.
...
4. **Drop low-confidence** — Remove findings where `verdict = "survives"` but `confidence < 70`. NOTE: sub-skills called via "Called from gauntlet orchestrator" path already pre-filter to confidence≥70; this substep is a no-op for those (preserved for safety). The v2 adversarial lane arrives pre-adjudicated (deterministic severity-stratified gate) — this substep is a no-op for it too. Only when the v1 fallback lane ran is this substep load-bearing (v1 emits raw findings regardless of caller).
...
- **For the adversarial lane (v2 default):** re-dispatch is NOT available — adjudication is deterministic inside the runtime and the lane returns pre-gated survivors; treat it like the black-box sub-skills below. Only when the v1 fallback lane ran does the old rule apply: re-dispatch the v1 skill once with prompt "apply false-positive rules from `code-quality-standards` more aggressively to the findings returned in the previous pass." (v1 has no gauntlet-caller JSON contract row in its Invocation Context Detection table, so its output is raw and re-dispatchable.)
...
- `adversarial-review` lane — dispatched via `gauntlet:gauntlet2-adversarial-review` (v2 runtime Class, default). Phase 1 dispatch target for code (`code-pr`, `code-local`) AND for plan/doc (family-named dispatch). `gauntlet:adversarial-review` (v1) is the explicit fallback, dispatched only on v2 preflight failure and footer-recorded.
