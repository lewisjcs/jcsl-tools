...
Write the `title` value to `"$STAGE/pr-title.md"` and the `body` value to `"$STAGE/pr-body.md"`. Set `AUTHOR=self` when `author.login` equals the `gh api user` login, else `AUTHOR=other`. When `gh repo view` succeeds, map its result to set `VISIBILITY` (`PUBLIC` → `public`, any other value → `private`): the runtime holds security findings from the posted comment on a public repository, so this is a fielding input the operator never edits. When it fails, leave `VISIBILITY` unset and tell the operator in chat: the runtime treats an unrecorded visibility the same as a public one, so it will hold security findings from any comment on this artifact.
...
When the operator says a reported party missed a defect — a reviewer, a later bug, or the operator's own reading found something the report did not — record it with `node "${CLAUDE_PLUGIN_ROOT}/runtime/bin/cli.mjs" escape --party <partyRunId> --lane <lane> --category <category> --note <text> [--ref <ref>]`: `--lane` is the Class that should have caught it, a Class key the runtime recognizes; `--category` is one of the runtime's finding categories; `--note` is one sentence saying what was missed; `--ref` is an optional pointer (a file:line, a commit, or a comment URL). The runtime appends the record to the party's `escapes.json` sidecar and prints the stamped record; it is only valid after `party-report` has run — `CLI_PARTY_NOT_REPORTED` is the refusal before that, and `CLI_INVALID_CLASS`, `ESCAPE_CATEGORY_INVALID`, `ESCAPE_NOTE_INVALID`, `ESCAPE_REF_INVALID`, `ESCAPE_SIDECAR_MALFORMED` are surfaced verbatim, never retried. The record is what later runs measure the Class against; the skill never runs `escape` on its own, only when the operator names a miss.
