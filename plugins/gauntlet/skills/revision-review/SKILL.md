...
3. **`next`** prints `{issues, next, hostBinding}`: `next` is the pending action, or `{"terminal": true}` once the run has reached `verified` or `gap`; `hostBinding` names the host agent to dispatch and the model it is bound to — dispatch to exactly that agent: `node "${CLAUDE_PLUGIN_ROOT}/runtime/bin/cli.mjs" next --state <runDir>/state.json`.
...
6. **`result`** produces the typed verify result and evidence record once the run is terminal: `node "${CLAUDE_PLUGIN_ROOT}/runtime/bin/cli.mjs" result --state <runDir>/state.json --out <runDir>/result.json --evidence <runDir>/evidence.json`.
