---
name: threat-review
description: Runtime-driven threat review of a code diff (Find, Validate, Adjudicate under a deterministic runtime): traces what the change newly trusts, from entry point to sink, and reports paths a named less-trusted party could exploit. Use when reviewing a code change for exploitable paths, threat-modelling a diff, or checking whether a change widens what the code trusts. Fielded by the gauntlet on a security signal; run it directly to force the lane. Trigger phrases include "threat review", "security review", "is this safe", "any security risks", "how could this be exploited", "secret leak", "auth bypass". Requires Node >= 22.
---
...
`next` prints the pending action as one JSON object, with fields including `actionId` (`dispatch-finder-<attempt>` or `dispatch-validator-<attempt>` — keep the exact value, attempt suffix included, in every file name below; hardcoding attempt `1` overwrites the previous attempt's prompt and raw-output files on a retry) and `promptBody`, the dispatch prompt string itself. Save `next`'s JSON to a file, e.g. `"$RUN_DIR/pending.json"`, then extract `promptBody` with `jq -r '.promptBody' "$RUN_DIR/pending.json"` — `-r` decodes the JSON string's escapes back to literal bytes, so embedded newlines, quotes, and backslashes survive intact; it appends one trailing newline, which is harmless. A plain `jq '.promptBody'` (no `-r`) or piping the field through `echo` instead re-quotes it and corrupts the prompt. The prompt directs the role to read the bundle from the run directory by reference; never paste or embed artifact content into it yourself.

Perform each dispatch as a fresh `codex exec` subprocess — never by continuing this conversation and never by spawning an in-session agent. For a `dispatch-finder` action, concatenate the finder role instructions at `"${CODEX_HOME:-$HOME/.codex}/roles/threat-finder.md"` and the extracted `promptBody`, in that order, into `"$RUN_DIR/<actionId>-prompt.md"`, then run: `codex exec -m gpt-5.6-terra -c model_reasoning_effort="medium" --sandbox read-only --skip-git-repo-check --ephemeral --output-last-message "$RUN_DIR/<actionId>-raw.json" - < "$RUN_DIR/<actionId>-prompt.md"`. For a `dispatch-validator` action, do the same with `"${CODEX_HOME:-$HOME/.codex}/roles/threat-validator.md"` and run: `codex exec -m gpt-5.6-terra -c model_reasoning_effort="medium" --sandbox read-only --skip-git-repo-check --ephemeral --output-last-message "$RUN_DIR/<actionId>-raw.json" - < "$RUN_DIR/<actionId>-prompt.md"`.
...
Class `jcsl:gauntlet:threat-review@1.0.0` — Runtime-driven threat review of a code diff (Find, Validate, Adjudicate under a deterministic runtime): traces what the change newly trusts, from entry point to sink, and reports paths a named less-trusted party could exploit. Two opposed roles (`jcsl:gauntlet:threat-finder`, `jcsl:gauntlet:threat-validator`) run in fresh, isolated dispatches under a deterministic runtime.
...
2. **`init`** admits the run and prints the first pending action: `node "${CODEX_HOME:-$HOME/.codex}/runtime/bin/cli.mjs" init --class threat-review --bundle <bundle.json> --family <artifactFamily> --host <claude-code|codex> --out <runDir>/state.json`.
