...
3. **`next`** prints `{issues, next, hostBinding}`: `next` is the pending action, or `{"terminal": true}` once the run has reached `audited` or `gap`; `hostBinding` names the host agent to dispatch and the model it is bound to — dispatch to exactly that agent: `node "${CODEX_HOME:-$HOME/.codex}/runtime/bin/cli.mjs" next --state <runDir>/state.json`.
