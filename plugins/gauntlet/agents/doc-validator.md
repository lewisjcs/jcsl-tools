...
4. Read relevant adjacent files referenced by the doc, sibling docs in the same directory, and the repo's CODEOWNERS/AGENTS.md if the finding hinges on convention. Use `Read`, `Grep`, and `Glob` aggressively. Confidence >85 requires evidence beyond the doc text.
