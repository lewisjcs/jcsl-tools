...
**Architectural risk lens (lens 5 in master spec §3.4) is not currently wired up.** It depended on gauntlet cross-dispatching `adversarial-review` with family `plan-text` against the plan content and relabeling the results as `plan-review / Architectural risk`; the run skill no longer does that cross-dispatch — plan-review is now a lane the Party runtime reports as a gap, offered as a follow-up the operator invokes directly. plan-finder itself does NOT emit Architectural-risk findings; that cross-skill dispatch path no longer exists. plan-finder's 5 lenses (EARS compliance, Internal consistency, Ambiguity, Scope, Test strategy) remain its sole emission set. Architectural-risk findings are not produced, standalone or via gauntlet.
