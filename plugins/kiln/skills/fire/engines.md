...
| **`finalize(task)`** | Inspector (STANDARD) / Crafter (TRIVIAL) | STANDARD → **Inspector** calls `implement_task_finalize` with the verdict evidence. TRIVIAL → **Crafter** calls `update_task(status="DONE")` (the Crafter is not granted `implement_task_finalize` — a TRIVIAL task needs no Compounds-project finalize) | STANDARD → **Inspector** calls `update_task(status="DONE")`. TRIVIAL → **Crafter** calls `update_task(status="DONE")` |
