...
| Cartographer provenance checker | `bash plugins/cartographer/scripts/check-grounding-provenance.sh` — ancestry-based ordering enforcement per RC-15/RC-24. Included in the `check-knowledge-grounding.test.sh` fixture suite. |
| Cartographer boundary checker | `bash plugins/cartographer/scripts/check-core-profile-boundary.sh` — boundary enforcement per RC-7. Included in the `check-knowledge-grounding.test.sh` fixture suite. |
| Cartographer local-validation checker | `bash plugins/cartographer/scripts/check-readme-patch.sh <README_FILE> [REPO_ROOT]` — fixture-driven unit tests via `bash plugins/cartographer/scripts/check-readme-patch.test.sh`. Validates link resolution, command existence, and low-value section flagging per `core/local-validation.md`. Note: `plugins/cartographer/scripts/fixtures/readme-patch/` contains synthetic `package.json`/`.github/workflows/` test manifests (permitted per RC-12); the repo-root invariant in § Prerequisites is unchanged. |
| Cartographer verification-report checker | `bash plugins/cartographer/scripts/check-verification-report.sh <VERIFICATION_REPORT_FILE>` — fixture-driven unit tests via `bash plugins/cartographer/scripts/check-verification-report.test.sh`. Validates the stage-5 verification report's record grammar and both gate predicates per `core/claim-verification.md` RC-31/RC-36. Its fixtures live in `plugins/cartographer/scripts/fixtures/accuracy-verification/` and `plugins/cartographer/scripts/fixtures/effectiveness-verification/`, one whole verification report per case. |
