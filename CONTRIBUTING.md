...
| Cartographer grounding checker | `bash plugins/cartographer/skills/cartograph-report/scripts/check-knowledge-grounding.sh` — fixture-driven unit tests via `bash plugins/cartographer/tests/check-knowledge-grounding.test.sh`. Validates rules (a), (b), (c), (c′), (d) per RC-1 through RC-5. Note: `plugins/cartographer/tests/fixtures/` contains synthetic test manifests (permitted per RC-12); the repo-root invariant in § Prerequisites is unchanged. |
| Cartographer provenance checker | `bash plugins/cartographer/skills/cartograph-report/scripts/check-grounding-provenance.sh` — ancestry-based ordering enforcement per RC-15/RC-24. Included in the `check-knowledge-grounding.test.sh` fixture suite. |
| Cartographer neutrality checker | `bash plugins/cartographer/skills/cartograph-report/scripts/check-core-neutrality.sh` — org-neutrality enforcement over the parity set (SKILL.md + core/ + scripts/): no org tokens, no unlicensed `profile/` references. Fixture cases run inside the `check-knowledge-grounding.test.sh` suite. Red on the real tree until the seam rewrite lands; exit 0 required before promotion. |
| Cartographer local-validation checker | `bash plugins/cartographer/skills/cartograph-report/scripts/check-readme-patch.sh <README_FILE> [REPO_ROOT]` — fixture-driven unit tests via `bash plugins/cartographer/tests/check-readme-patch.test.sh`. Validates link resolution, command existence, and low-value section flagging per `core/local-validation.md`. Note: `plugins/cartographer/tests/fixtures/readme-patch/` contains synthetic `package.json`/`.github/workflows/` test manifests (permitted per RC-12); the repo-root invariant in § Prerequisites is unchanged. |
